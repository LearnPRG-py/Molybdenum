...
def configure():
    ...
    if repo not in supported_repos:
        print("Repo is currently not supported, you could try push a PR to" \
        "add support, create an issue or wait till the repo is supported." \
        "thank you for your patience!")
        quit()
    print("\n"*5)
    directory = input("Enter the directory of your chromium checkout (src folder).")
    print("\n"*5)
    print("# Supervision level determines how updates work: \
# 0. No supervision: Just writes images to your git worktree, but you can revert if needed \
# 1. Confirm at the end: Confirms updates at the end of processing and prior to writes \
# 2. Confirms for each individual image with a visual difference shown.")
    ...
    try:
        ...
        quit()
    file = '# Setting for which repo you are performing the "roll" in. \n repo = "'+repo+'"\n\n# Where that repository is present\ndirectory = "'+directory+'"\n\n# Supervision level determines how updates work:\n# 0. No supervision: Just writes images to your git worktree, but you can revert if needed\n# 1. Confirm at the end: Confirms updates at the end of processing and prior to writes\n# 2. Confirms for each individual image with a visual difference shown.\nsupervision_level = '+supervision_level_str+'\n'
    config_file.write(file)
    config_file.close()

def get_logs():
    print("\n"*20)
    print("To get the error log, find the failing test suite and paste the error here from the stdout section.")
    print("The test suites currently supported are:", list(supported_test_classes))
...
def main():
    ...
    change_config = not (input("Would you like to go ahead with this configuration?").lower() == "y")
    ...
    for i, line in enumerate(lines):
        if "Actual pixels (open in browser):" in line:
            if (i+6 < len(lines)):
                ...
                output = os.path.join(config["directory"], suite_folder_mapping[test_suite], img_name)
                with open(output, "wb") as f:
                    f.write(image_bytes)
                print("["+str(processed_count)+"/"+str(count)+"] - "+img_name)
